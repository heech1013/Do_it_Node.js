...
function connectDB(){  // 데이터베이스 연결 함수 정의
    ...
    myDatabase.on('open', ()=>{  // open 이벤트: 데이터베이스 연결 시 발생
        ...
        UserSchema = mongoose.Schema(  // 스키마(컬렉션이 가지는 속성) 정의
            {
                ...
                name: {type:String, index:'hashed'},  // index:'hashed': 인덱스 형성
        ...
        UserSchema.statics.findById = function(id, callback){  // static(): 스키마 객체에 메소드 추가. param1:메소드 이름, param2:호출될 함수 / 인스턴스 객체에 메소드 추가는 method()
            return this.find({id:id}, callback);
        };
        UserSchema.statics.findAll = function(callback){  // ***mongoose에서는 fat-arrow syntax를 사용하면 안된다. this가 mongoose에서 다른 것을 가리킬 수 있기 때문이다.
            return this.find({}, callback);
        };
        console.log('UserSchema 정의함.');

        UserModel = mongoose.model("users2", UserSchema);  // UserModel 모델(데이터베이스의 컬렉션 지정) 정의. 1param:'모델이름'(데이터베이스의 같은 이름의 컬렉션과 매칭된다), 2param:스키마 객체
